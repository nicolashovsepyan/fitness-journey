...
/* ============================================================
   ...
   IT IS ONE IMAGE, AND IT WAS BRIEFLY TWO. A real rack changes its balls
   enormously between 4 kg and 40 kg while its handles barely change, because
   every one of them has to fit the same hand — so the artwork was split into
   ball and handle and scaled on separate laws. It worked, and it was not
   worth it: side by side at card size the difference was very hard to see,
   while a light bell's oversized handle could run out of ball to hide its
   legs in. One picture, scaled whole, is the version with nothing to go
   wrong. If that decision is ever revisited, the split is recoverable from
   git — look for kettlebell-ball.png and kettlebell-handle.png.
...
/* MASS -> LENGTH. Same cube-root law as the plates: 20x the weight is 2.71x
   the bell, not 20x. `growth` is exposed so it can be damped — a real rack
   runs a little under cube root, because the number on the label includes a
   handle whose weight barely changes. */
...
function kettlebellSceneSVG(weight, o){
  ...
  /* `wide` and `tall` are a deliberate pair of separate multipliers rather
     ...
     that reads too narrow for its height, and a bell that reads too small
     for its slot. Both at 1 is the artwork's own proportion. */
...
/* The kettlebell has no yaw because it is a fixed render, not geometry — it
   ...
   `src` IS RELATIVE TO THE PAGE, not to this module. The app's pages sit at
   the site root so the default is right for them; the lab lives in logo/ and
   passes '../images/…'. It is not root-relative '/images/…' because the site
   is served from a subpath and that would 404 there. A missing image does not
   throw — it draws nothing — so a page that gets this wrong loses its
   kettlebell silently. */
export var KETTLEBELL = {
  ...
  numbers: true,
  numX: 0.7963,       /* medallion centre, fraction of the drawn box — measured */
  numY: 0.7031,
