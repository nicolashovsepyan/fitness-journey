...
/* How far the neon bloom carries past the stroke. The drop shadows below run
   stdDeviation 11 and 3; a gaussian is spent by about three sigma, so 33 is
   the honest reach rather than the 18 this file used to assume. */
export const BLOOM = 33;
...
/* ---- the app icon --------------------------------------------------------
   ...
   triangle. */
/* THE STROKE, AND WHY IT IS NO LONGER THE HERO'S.

   This file spent a long time trying to keep the icon's proportions faithful
   to the moving mark: the hero's outline measures 2.86px against the outer
   plate, the widest plate and the bar alike, and 4.5 was a compromise around
   that number.

   It was the wrong thing to be faithful to. A 512 tile is drawn at 60px on a
   home screen, so 4.5 units land at HALF A PIXEL — the outline was not thin,
   it was sub-pixel, and it read as a wireframe rather than a sign. The hero
   is a 400px animation on a dark screen and can carry a hairline; an icon
   cannot. They are different objects at different sizes and they are allowed
   to differ.

   20 is 2.34px at home-screen size. The triangle also grew from 1.42 to 1.60,
   which takes the mark from 83% of the tile to 94%, and the dumbbell rides
   higher (cy 214 -> 196) so it can grow with it: the triangle NARROWS as it
   descends, so height is what buys the plates room. Measured ink is now 45%
   of the tile against 35% before.

   iconClearance() is what keeps this honest — see the note above it. The
   build refuses under 8px and this sits at 10. */
export const ICON_LAYOUT = {
  tri: { cx: 256, cy: 272, scale: 1.60, stroke: 20 },
  db:  { cx: 256, cy: 196, scale: 1.426 },
};
...
/* THE MARK ALONE — no tile, transparent, cropped to the artwork.

   The app's own screens (the claim screen and the top of Week) had been
   showing images/logo.png since June: a wide italic "FitnessJourney"
   wordmark with a stick figure, on a white band. A different logo from a
   different era of this product, sitting inside an app whose every other
   surface is the neon mark.

   appIconSVG() is the same drawing but it paints TILE behind it, which is
   right for a home-screen icon and wrong anywhere inside the app. Rather
   than a second drawing, this is that function without the rect and with a
   viewBox pulled in to the artwork.

   The box was hand-derived from a 1.42 scale and carried the note "if
   ICON_LAYOUT moves, this moves with it" — which is exactly the kind of
   instruction that gets missed. It moved, to 1.60, and the old box would have
   cropped the mark on the claim screen and the top of Week while the icon
   itself looked right. So the box is COMPUTED now: the triangle's extent under
   whatever transform ICON_LAYOUT specifies, plus half the stroke, plus the
   bloom. Nothing to remember. */
export function markBox(layout = ICON_LAYOUT) {
  const { tri } = layout;
  const P = u => [(u[0] - 150) * tri.scale + tri.cx, (u[1] - 134) * tri.scale + tri.cy];
  const pts = [P([14, 16]), P([286, 16]), P([150, 252])];
  const pad = tri.stroke / 2 + BLOOM;
  const xs = pts.map(p => p[0]), ys = pts.map(p => p[1]);
  const x = Math.min(...xs) - pad, y = Math.min(...ys) - pad;
  return { x: +x.toFixed(1), y: +y.toFixed(1),
           w: +(Math.max(...xs) + pad - x).toFixed(1),
           h: +(Math.max(...ys) + pad - y).toFixed(1) };
}
