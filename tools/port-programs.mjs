...
function presc(it, block) {
  ...
  /* a circuit states its rounds on the block, so the item only says the work */
  if (!it.sets && block.rounds && it.hold == null && reps != null && !it.repsText)
    base = `${reps}`;
  ...
  return {
    ...
    blocks: ses.blocks.map((b, i) => ({
      ...
      items: b.items.map(it => {
        ...
        return { ex: it.ex, pres: presc(it, b), note: note(it) || '' };
      })
    }))
  };
}
